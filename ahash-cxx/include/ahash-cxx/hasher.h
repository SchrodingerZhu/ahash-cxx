...
class VectorizedHasher
{
  ...
  template <typename Accelerator, bool EXPECT_WIDE>
  bool
...
#  if AHASH_CXX_HAS_WIDER_SIMD_ACCELERATION
                if (length > 1024)
                  {
                    if (vectorized_consume<WideVectorOperator, true> (data, length))
                      {
                        return;
                      }
                  }
#  endif
                vectorized_consume<VectorOperator, false> (data, length);
              }
            else
              {
